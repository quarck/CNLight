...
//        }
    ...
    fun deleteAlert(entry: MonitorEventAlertEntry)
            = synchronized(CalendarMonitorStorage::class.java) { writableDatabase.use { impl.deleteAlert(it, entry) } }
    ...
    fun deleteAlert(a: Int, b: Int, c: Int, d: Int, alertTime: Long, instanceStart: Long)
            = synchronized(CalendarMonitorStorage::class.java) { writableDatabase.use { impl.deleteAlert(it, a, b, c, d, alertTime, instanceStart) } }
    ...
    fun getAlert(md5: md5state, alertTime: Long, instanceStart: Long): MonitorEventAlertEntry?
            = synchronized(CalendarMonitorStorage::class.java) { readableDatabase.use { impl.getAlert(it, md5, alertTime, instanceStart) } }

    fun getInstanceAlerts(md5: md5state, instanceStart: Long): List<MonitorEventAlertEntry>
            = synchronized(CalendarMonitorStorage::class.java) { readableDatabase.use { impl.getInstanceAlerts(it, md5, instanceStart) } }
